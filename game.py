...
class TicTacToe:
    ...
    def startTraining(self,player1,player2):
        if(isinstance(player1,learning) and isinstance(player2, learning)):
            self.training = True
    ...
    def startGame(self, playerX, playerO):
        if (isinstance(playerX, Human)):
            self.humman, self.computer = True, False
            if (isinstance(playerO, learning)): #if AI
                self.ai = playerO
        ...
        elif (isinstance(playerO, Human)):
            self.humman, self.computer = False, True
            if (isinstance(playerX, learning)): #if AI
                self.ai = playerX
    ...
    def render(self):
        ...
        while (running == 1):
            if (self.humanTurn): #humman click
                ...
                if(self.isAI):
                    moves = self.ai.epsilon_greedy(self.board, self.possible_moves())
